    ...
    <div className="bg-black text-white min-h-screen px-4 py-10">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside className="lg:col-span-1 bg-[#121212] rounded-xl p-4 shadow">
          <h2 className="text-lg font-bold mb-3 text-[#B08D57] ">
            Ligas populares
          </h2>
          <ul className="space-y-5 text-sm">
            {ligas.slice(0, 6).map((liga) => (
              <li
                key={liga.id}
                className="flex items-center gap-2 cursor-pointer hover:text-[#B08D57] hover:scale-105 transition-transform duration-300"
              >
                <img
                  src={liga.logo}
                  alt={liga.nombre}
          ...
          <Link to="/ligas">
            <button className="mt-4 bg-[#4CCC6C] text-white text-sm px-4 py-1 rounded-full hover:bg-[#B08D57] cursor-pointer">
              Ver todas las ligas
            </button>
          </Link>
        </aside>
        ...
        <main className="lg:col-span-2 space-y-6">
          <div className="bg-[#1C1C1C] rounded-xl p-4 flex flex-wrap items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <button className="text-white bg-[#333] hover:bg-[#444] w-8 h-8 flex items-center justify-center rounded-full">
                &lt;
              </button>
              <DatePicker
            ...
            <section key={liga} className="bg-[#1C1C1C] rounded-xl p-4">
              ...
              <div className="space-y-2">
                {juegos.map((p) => (
                  <div
                    key={p.idEvento}
                    className="flex justify-between items-center bg-[#151515] px-4 py-2 rounded text-sm"
                  >
        ...
        <aside className="lg:col-span-1 space-y-6">
          <section className="bg-[#1C1C1C] rounded-xl p-4">
            <h2 className="text-[#B08D57] font-bold mb-2">
              Equipos Destacados
            </h2>
          ...
          <section className="bg-[#1C1C1C] rounded-xl p-4">
            <h2 className="text-base font-bold mb-2 text-[#B08D57]">
              Noticias Rápidas
            </h2>
            <ul className="space-y-2 text-sm">
