        ...
        {equipo ? (
          <>
            <div className="bg-[#1C1C1C] rounded-xl p-6 flex flex-col items-center text-center">
              <img src={equipo.logo} alt={equipo.nombre} className="w-24 h-24 object-contain mb-4" />
              <h1 className="text-3xl font-bold text-[#B08D57]">{equipo.nombre}</h1>
              <p className="text-gray-400">{equipo.pais}</p>
              <p className="text-sm text-[#B08D57]">{liga}</p>
            </div>

            <section className="bg-[#121212] rounded-xl p-6">
              <h2 className="text-xl font-bold text-[#B08D57] mb-4">Plantilla</h2>
              {jugadores.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {jugadores.map((jugador) => (
                    <div key={jugador.id} className="bg-[#1C1C1C] rounded-lg p-3 text-center">
                      <img
                        src={jugador.foto}
                        alt={jugador.nombre}
                        className="w-16 h-16 mx-auto rounded-full object-cover mb-2"
                      />
                      <h3 className="text-sm font-semibold">{jugador.nombre}</h3>
                      <p className="text-xs text-gray-400">#{jugador.numero} | {jugador.posicion}</p>
                    </div>
                  ))}
                </div>
              ) : (
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  <svg
                    ...
                    className={`w-4 h-4 transition-transform ${ordenAscendente ? "rotate-180" : ""}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
          ...
          <p className="text-center text-gray-400">Cargando datos del equipo...</p>
        )}
      </div>
    </div>
  );
